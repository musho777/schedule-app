...
function formatSelected(dateStr) {
  ...
  if (
    dateStr ===
    todayKey().replace(/\d+$/, (n) => String(Number(n) - 1).padStart(2, "0"))
  )
    return "Yesterday";
...
export default function ProjectView({ project }) {
  ...
  const dayNum = project.checklistMode
    ? null
    : dayForDate(startDate, selectedDate);
  ...
  const doneCount = allTasks.filter((t) =>
    completedForSelected.includes(t.id),
  ).length;
  ...
  return (
    <div className={s.app}>
      <header className={s.header}>
        ...
        {!project.checklistMode && (
          <div className={s.headerRight}>
            ...
            >
              {currentDay
                ? `Day ${currentDay} / ${project.duration}`
                : `Day — / ${project.duration}`}
            </span>
            {startDate && (
              <button
                className={s.analyticsBtn}
      ...
      {quizTask && (
        <QuizModal task={quizTask} onClose={() => setQuizTask(null)} />
      )}
      {lcTask && (
        <LeetCodeModal task={lcTask} onClose={() => setLcTask(null)} />
      )}
      ...
      <main
        className={
          project.checklistMode || project.listMode ? s.mainFull : s.main
        }
      >
        {project.checklistMode ? (
          <ChecklistView
          ...
          <>
            ...
            {dayData && (
              <div className={s.weekCard}>
                <h2 className={s.weekCardTitle}>
                  Day {dayNum} — Week {dayData.week}
                </h2>
                <p className={s.weekCardDesc}>
                  Month {dayData.month} of 3 ·{" "}
                  {allTasks.length - project.dailyTasks.length} focused tasks
                  today
                </p>
              </div>
            )}
