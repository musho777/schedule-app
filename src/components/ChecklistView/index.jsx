...
export default function ChecklistView({
  ...
  const allTaskIds = project.categories?.flatMap((c) =>
    c.tasks.map((t) => t.id),
  );
  ...
  const filteredCategories = project.categories
    ?.map((cat) => ({
      ...cat,
      tasks: q
        ? cat.tasks.filter((t) => t.title.toLowerCase().includes(q))
        : cat.tasks,
    }))
  ...
  return (
    <div className={s.container}>
      ...
      {filteredCategories?.map((category) => {
        const catDone = category.tasks.filter((t) =>
          ...
          <div key={category.id} className={s.category}>
            ...
            >
              ...
              <div className={s.categoryRight}>
                <span
                  className={`${s.categoryCount} ${allDone ? s.allDone : ""}`}
                >
                  {catDone}/{catTotal}
                </span>
                <svg
            ...
            <div
              className={`${s.taskList} ${isCollapsed ? s.taskListCollapsed : ""}`}
            >
